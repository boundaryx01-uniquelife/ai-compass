...
이 30개 PASS는 지정된 v1 인수 기준 충족을 뜻하며, 모든 향후 데이터 변경이나 캐시 버전 변경의 운영 누락까지 검증했다는 뜻은 아닙니다. 비교 순서 변경과 매핑 오류는 별도의 회귀 검증으로 확인합니다.
...
`mapping-regression.cjs`로 원본 파일을 수정하지 않고 브라우저 응답만 대체해 확인했습니다. 결과: 6 PASS / 0 FAIL (`mapping-regression-results.json`).

| 항목 | 결과 |
|---|---|
| 비교 배열 역순에서도 15개 URL·내용 및 자율성 연결 유지 | PASS |
| 알 수 없는 비교 라벨 오류 표시 | PASS |
| 중복 비교 및 그에 따른 누락 오류 표시 | PASS |
| 새 퀴즈 ID에 복습 매핑이 없으면 오류 표시 | PASS |
| 삭제된 퀴즈의 남은 매핑 오류 표시 | PASS |
| 비교 메타데이터의 없는 CORE 참조 오류 표시 | PASS |

서비스 워커 캐시 버전은 v1-3으로 올렸습니다. 향후 배포 시 수동 버전 변경을 누락하는 운영 위험은 이 검증으로 제거되지 않습니다.
